...
class CustomOpenAPISchemaGenerator(OpenAPISchemaGenerator):
    def get_operation_id(self, operation_keys):
        ...
        method = (
            operation_keys[-1]
            if operation_keys[-1] in ["get", "post", "patch", "put", "delete"]
            else "get"
        )
    ...
    def get_operation(self, view, path, prefix, method, components, request, **kwargs):
        ...
        operation = super().get_operation(
            view, path, prefix, method, components, request, **kwargs
        )
...
urlpatterns = [
    ...
    path(
        ...
        TemplateView.as_view(
            template_name="main/assetlinks.json", content_type="application/json"
        ),
    ),
    ...
    path(
        "webhooks/telegram/",
        csrf_exempt(TelegramBotView.as_view()),
