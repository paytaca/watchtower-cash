# Generated by Django 3.0.14 on 2024-08-28 04:52
...
'''
Marks canceled orders as already read by order members
'''
def read_canceled_orders(apps, schema_editor):
    Order = apps.get_model('rampp2p', 'Order')
    Status = apps.get_model('rampp2p', 'Status')

    last_status = Status.objects.filter(
        order=OuterRef('pk'),
        status__in=[StatusType.CANCELED]
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('rampp2p', '0182_auto_20240822_0524'),
    ]

    operations = [
        migrations.RunPython(read_canceled_orders)
    ]
