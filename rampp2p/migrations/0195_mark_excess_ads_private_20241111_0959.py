# Generated by Django 3.0.14 on 2024-11-11 09:59
...
def get_ads_by_peer(queryset, fiat_currency, trade_type):
    ...
    queryset = queryset.filter(trade_type=trade_type, fiat_currency__id=fiat_currency.id)
    if trade_type == 'BUY':
...
def mark_excess_ads_private(apps, schema_editor):
    ...
    queryset = queryset.annotate(market_rate=Subquery(MarketRate.objects.filter(currency=OuterRef('fiat_currency__symbol')).values('price')[:1]))
