...
export class WatchtowerNetworkProvider {
     ...
     * @param {string} payload.addressIndex Example: "0/0"
     * @param {string} [payload.walletHash]
     * @returns {Promise<{ success: boolean, txid: string }>}
     */
    async subscribeAddress({ address, addressIndex, walletHash }) {
        const projectId = {
        ...
        // if (addressIndex) {
        //     payload.wallet_index = 0
        //     payload.address_index = addressIndex
        // }
        if (walletHash) {
            payload.wallet_hash = walletHash
        }
        return await axios.post(`${this.hostname}/api/multisig/wallets/subscriptions/`, payload)
    }
}
