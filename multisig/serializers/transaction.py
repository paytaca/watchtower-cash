...
class InputSerializer(serializers.ModelSerializer):
    ...
    redeemScript = serializers.CharField(
        source="redeem_script", read_only=True, allow_blank=True, allow_null=True
    )
    spendingTxid = serializers.CharField(
        source="spending_txid", read_only=True, allow_blank=True, allow_null=True
    )
    conflictingProposalIdentifier = serializers.CharField(
        source="conflicting_proposal_identifier",
        read_only=True,
        allow_blank=True,
        allow_null=True,
    )
...
class ProposalSerializer(serializers.ModelSerializer):
    ...
    proposalFormat = serializers.CharField(
        source="proposal_format", default="psbt", required=False, allow_blank=True
    )
    unsignedTransactionHex = serializers.CharField(
        source="unsigned_transaction_hex", read_only=True
    )
    unsignedTransactionHash = serializers.CharField(
        source="unsigned_transaction_hash", read_only=True
    )
    signedTransaction = serializers.CharField(
        source="signed_transaction", read_only=True
    )
    signedTransactionHash = serializers.CharField(
        source="signed_transaction_hash", read_only=True
    )
    ...
    coordinatorProposalSignature = serializers.CharField(
        source="coordinator_proposal_signature",
        required=False,
        allow_blank=True,
        allow_null=True,
    )
    coordinatorProposalSignatureScheme = serializers.CharField(
        source="coordinator_proposal_signature_scheme", required=False, allow_blank=True
    )
        ...
        with transaction.atomic():
            ...
            if proposal_format == "psbt":
                ...
                if created:
                    psbt = None
                    if (
                        signing_progress.get("signingProgress", "")
                        and signing_progress.get("signingProgress", "") != "unsigned"
                    ):
                    ...
                    for input in inputs:
                        input_model = Input.objects.create(
                            proposal=proposal,
                            outpoint_transaction_hash=input.get(
                                "outpointTransactionHash"
                            ),
                        ...
                        for pub_key, derivation in bip32_derivation.items():
                            ...
                            Bip32Derivation.objects.get_or_create(
                                ...
                                defaults={
                                    ...
                                    "master_fingerprint": derivation.get(
                                        "masterFingerprint"
                                    ),
                                },
                            )
                        ...
                        for public_key, signature in signatures.items():
                            if not bip32_derivation.get(public_key):
                                raise ValidationError(
                                    f"BIP32 derivation data is missing for public key: {public_key}"
                                )
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            ...
            if standard == "psbt":
                ...
                response = js_client.combine_psbts(
                    [proposal.combined_psbt, validated_data["content"]]
                )
                response.raise_for_status()
                response_json = response.json()

                if response_json.get("combinedPsbt"):
                    proposal.combined_psbt = response_json.get("combinedPsbt")
                    signing_progress = response_json.get("signingProgress", {})
                    proposal.signing_progress = signing_progress.get("signingProgress")
                    proposal.save(update_fields=["combined_psbt", "signing_progress"])
                ...
                for input in inputs:
                    ...
                    if not input_model:
                        raise ValidationError(
                            f"Signed an input that's not part of the proposal"
                        )
...
class Bip32DerivationSerializer(serializers.ModelSerializer):
    ...
    masterFingerprint = serializers.CharField(
        source="master_fingerprint", read_only=True, allow_null=True
    )
